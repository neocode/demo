...
overlayImg.src = 'masks/onix_02.png'
...
function onResults(results) {
  ...
  if (results.multiFaceLandmarks) {
    for (const landmarks of results.multiFaceLandmarks) {
      const maskWidth = Math.round((landmarks[454]['x'] - landmarks[234]['x'])*canvasElement.width) + 40;
      const xLeft = Math.round(landmarks[195]['x']*canvasElement.width - maskWidth/2);
      const yLeft = Math.round(landmarks[195]['y']*canvasElement.height) - 20;
      const maskHeight = Math.round((landmarks[152]['y'] - landmarks[94]['y'])*canvasElement.height) + 100;
      // console.log(xLeft);
